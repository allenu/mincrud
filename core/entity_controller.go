...
func (sc EntityController) Update(user User, adminMode bool, modifiedEntity model.Entity, modifiedFields []string) (model.Entity, error) {
    ...
    if modifiedEntity.EntityId != "" {
        ...
        if !sc.authorizer.CanWrite(user, adminMode, storedEntity) {
            return storedEntity, errors.New("User has no write access for new entities")
        }
    }
...
func (sc EntityController) ReadEntities(user User, adminMode bool) ([]model.Entity, error) {
    var filteredEntities []model.Entity = []model.Entity{}
    var entities []model.Entity
    var err error

    entities, err = sc.db.ReadEntities()
    if err == nil {
    ...
    return filteredEntities, err
}

func (sc EntityController) ReadEntitiesForUser(user User, adminMode bool, userId string) ([]model.Entity, error) {
    var filteredEntities []model.Entity = []model.Entity{}
    var entities []model.Entity
    var err error

    entities, err = sc.db.ReadEntitiesForUser(userId)
    if err == nil {
        for _, entity := range entities {
            if sc.authorizer.CanRead(entity, user, adminMode) {
                filteredEntities = append(filteredEntities, entity)
